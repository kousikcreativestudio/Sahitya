...
const TEXT_FORM_MS = 1900;
const SECOND_GIFT_DELAY_MS = 7600;
const BLUR_BEFORE_GIFT_MS = 6900;
...
function startMusic(){
  ...
  function loop(){ tone(notes[i%notes.length],1.05,'sine',.028); tone(notes[(i+2)%notes.length]*2,.55,'triangle',.014,.13); i++; }
  loop(); musicTimer = setInterval(loop,1250);
}

function updateScore(){ score.textContent = `Stars ${collected} / ${TARGET_STARS}`; }
...
function spawnStar(initial=false){
  ...
  star.style.width=size+'px'; star.style.height=size+'px'; star.style.fontSize=(size*.88)+'px';
  ...
  const trail=document.createElement('div'); trail.className='trail'; star.appendChild(trail);
  star.addEventListener('pointerdown', e=>{
    e.preventDefault();
    if(!gameRunning) return;
    const r=star.getBoundingClientRect();
    star.remove(); collected++; updateScore(); collectSound(); sparkBurst(r.left+r.width/2,r.top+r.height/2,16);
...
function startGame(){
  initAudio(); show('game'); collected=0; updateScore(); gameLayer.innerHTML=''; makeAmbientStars();
  gameRunning=true;
...
function sparkBurst(x,y,n=12){
  for(let i=0;i<n;i++){
    const sp=document.createElement('div'); sp.className='spark'; sp.style.left=x+'px'; sp.style.top=y+'px';
    sp.style.setProperty('--x',(Math.random()*150-75)+'px'); sp.style.setProperty('--y',(Math.random()*150-75)+'px');
    app.appendChild(sp); setTimeout(()=>sp.remove(),820);
  }
}
function flashOpen(){ flash.classList.remove('show'); void flash.offsetWidth; flash.classList.add('show'); }
...
function textPoints(w,h){
  ...
  const c=off.getContext('2d'); c.fillStyle='white'; c.textAlign='center'; c.textBaseline='middle';
  ...
  const pts=[]; const gap=Math.max(3,Math.floor(w/150));
...
function drawStarParticle(x,y,r,a,cross){
  ctx.save(); ctx.globalAlpha = a; ctx.shadowColor='#ffd55d'; ctx.shadowBlur=14;
  if(cross){
    ctx.strokeStyle='rgba(255,242,188,.96)'; ctx.lineWidth=Math.max(.65,r*.24);
    ctx.beginPath(); ctx.moveTo(x-r*1.4,y); ctx.lineTo(x+r*1.4,y); ctx.moveTo(x,y-r*1.4); ctx.lineTo(x,y+r*1.4); ctx.stroke();
  }
  ctx.beginPath(); ctx.fillStyle='rgba(255,238,170,.98)'; ctx.arc(x,y,r*.55,0,Math.PI*2); ctx.fill(); ctx.restore();
}
function startStarText(){
  resizeCanvas(); textParticles=[];
  ...
  textStartTime = performance.now(); textAnimating=true; animateStarText();
}
function animateStarText(now=performance.now()){
  ...
  if(completeCount===textParticles.length){
    // extra random twinkles after letters are fully formed
    for(let i=0;i<14;i++){
...
function openWish(){
  show('wish'); screens.wish.classList.remove('blur'); secondGift.classList.remove('show'); secondLabel.classList.remove('show');
  startStarText();
  setTimeout(()=>revealSound(),900);
  setTimeout(()=>screens.wish.classList.add('blur'),BLUR_BEFORE_GIFT_MS);
  setTimeout(()=>{ secondGift.classList.add('show'); secondLabel.classList.add('show'); },SECOND_GIFT_DELAY_MS);
}
function startPhotos(){
  show('photo'); photoStage.innerHTML=''; photoCount.textContent='';
  const list = photos.length ? photos : ['__placeholder__'];
  showPhoto(0,list);
}
function showPhoto(i,list){
  if(i>=list.length){ show('final'); return; }
  photoSound(); photoStage.innerHTML='';
  const card=document.createElement('div'); card.className='photoCard';
  if(list[i]==='__placeholder__'){
    const ph=document.createElement('div'); ph.className='placeholder'; ph.innerHTML='Add your selected photos later ✨'; card.appendChild(ph); photoCount.textContent='';
  } else {
    const img=document.createElement('img'); img.src=list[i]; img.alt='Birthday memory photo';
    img.onerror=()=>{ img.replaceWith(Object.assign(document.createElement('div'),{className:'placeholder',innerHTML:'Photo not found<br>'+list[i]})); };
    card.appendChild(img); photoCount.textContent=`${i+1} / ${list.length}`;
  }
  photoStage.appendChild(card); sparkBurst(app.clientWidth/2,app.clientHeight/2,26);
  setTimeout(()=>showPhoto(i+1,list),5100);
}

playBtn.addEventListener('click', startGame);
firstGift.addEventListener('click', ()=>{ initAudio(); boxSound(); firstGift.classList.add('open'); flashOpen(); setTimeout(()=>{ firstGift.classList.remove('open'); openWish(); },850); });
secondGift.addEventListener('click', ()=>{ initAudio(); boxSound(); secondGift.classList.add('open'); flashOpen(); setTimeout(()=>{ secondGift.classList.remove('open'); startPhotos(); },780); });
replayBtn.addEventListener('click', ()=>{
  textAnimating=false; ctx.clearRect(0,0,canvas.width,canvas.height);
  screens.wish.classList.remove('blur'); secondGift.classList.remove('show'); secondLabel.classList.remove('show');
  show('start');
});
